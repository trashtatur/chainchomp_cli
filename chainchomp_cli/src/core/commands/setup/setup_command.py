...
def setup():
    ...
    with click.progressbar(
        ...
        click.echo(style('Creating folder for installed adapters if it does not already exist', fg='cyan'))
        created_adapters_folder = AdapterFolderHandler.create_adapter_folder()
        if created_adapters_folder:
            click.echo(style('Created adapters folder in path:', fg='cyan'))
            click.echo(style(f'{PathProvider.installed_adapters_folder()}', fg='cyan'))
        if not created_adapters_folder:
            click.echo(style('Adapters folder already exists. Moving on', fg='blue'))
        bar.update(1)

        click.echo(style('Creating folder for fixtures if it does not already exist', fg='cyan'))
        created_fixtures_folder = FixturesFolderHandler.create_fixtures_folder()
        if created_fixtures_folder:
            click.echo(style('Created fixtures folder in path:', fg='cyan'))
            click.echo(style(f'{PathProvider.fixtures_folder()}', fg='cyan'))
        if not created_fixtures_folder:
            click.echo(style('Fixtures folder already exists. Moving on', fg='blue'))
        bar.update(1)

    click.echo(style('Setup concluded', fg='green'))
