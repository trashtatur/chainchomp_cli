...
class ConfigWriterHandlerTest(unittest.TestCase):
    ...
    def test_writing_successful(self):
        ChainfileWriterHandler.write_chainfile(self.chainlinkModel, self.configPath, False)
    ...
    def test_path_does_not_exist(self, path, expected):
        result = ChainfileWriterHandler.write_chainfile(self.chainlinkModel, path, False)
        assert expected == result

    def test_file_already_exists(self):
        open(os.path.join(self.configPath, 'chainfile.yml'), 'x')
        result = ChainfileWriterHandler.write_chainfile(self.chainlinkModel, self.configPath, False)
        assert result is None

    def test_file_is_overwritten_on_force(self):
        open(os.path.join(self.configPath, 'chainfile.yml'), 'x')
        result = ChainfileWriterHandler.write_chainfile(self.chainlinkModel, self.configPath, True)
